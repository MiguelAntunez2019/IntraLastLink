...
namespace Ordenes
{
    public class Program
    {
        ...
        public async Task EjecutaPrograma()
        {
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        foreach (Woocomerce.Order order in orders)
                        {
                            ...
                            dto.comuna = order.shipping.city?.ToString() ?? "";
                            dto.nombre = order.shipping.first_name?.ToString() ?? "";
                            dto.apellidos = order.shipping.last_name?.ToString() ?? "";
                            dto.correo = order.shipping.email?.ToString() ?? "";
                            dto.nota_orden = order.customer_note?.ToString() ?? "";
                            dto.telefono = order.shipping.phone?.ToString() ?? "";
                            dto.courrier = "";
                            dto.envio = "";
                            dto.direccion = order.shipping.address_1 ?? "";
                            dto.compania = order.shipping.company?.ToString() ?? "";
                    ...
                    try
                    {
                        ...
                        if (response.IsSuccessStatusCode)
                        {
                            ...
                            foreach (var order in root.Orders)
                            {
                                ...
                                if (!string.IsNullOrEmpty(order.name) && order.name[0] == '#')
                                {
                                    datos.ordenid = order.name.Substring(1);
                                }
                                else
                                {
                                ...
                                datos.nombre = order.customer.default_address?.first_name ?? "";
                                datos.apellidos = order.customer.default_address?.last_name ?? "";
                                datos.comuna = order.customer.default_address?.city ?? "";
                                var direccion = order.customer.default_address.address1 + " " + order.customer.default_address.address2;
                                datos.direccion = direccion ?? "";
                                datos.compania = "";
                                datos.correo = order.email ?? "";
                                datos.telefono = order.customer.default_address?.phone ?? "";
                                ...
                                foreach (var item in order.Line_Items)
                                {
                                    ...
                                    InsertaProductos(datos.estado, datos_producto.nombre_producto, datos_producto.sku, datos_producto.cantidad_vendida.ToString(), datos.ordenid, datos_producto.pid, datos_producto.url);
                    ...
                    try
                    {
                        ...
                        if (response.IsSuccessStatusCode)
                        {
                            ...
                            foreach (var order in root.Orders)
                            {
                                ...
                                datos.origen = "Shopify";
                                datos.nombre = order.customer.default_address?.name ?? "";
                                datos.apellidos = order.customer.default_address?.last_name ?? "";
                                datos.comuna = order.customer.default_address?.city ?? "";
                                var direccion = order.customer.default_address.address1 + " " + order.customer.default_address.address2;
                                datos.direccion = direccion ?? "";
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        foreach (var item in orderWrappers)
                        {
                            ...
                            datos.numero_orden = item.Order.id.ToString() ?? "0";
                            if (item.Order.id.ToString() == "14193")
                            {
                            }
                            ...
                            datos.telefono = item.Order.customer?.phone ?? "";
                            if (item.Order.shipping_option.ToLower() == "delivery")
                            {
                                datos.envio = "Despacho";
                            }
                            else if (item.Order.shipping_option.ToLower() == "store_pickup")
                            {
                                datos.envio = "Retiro en tienda";
                            }
